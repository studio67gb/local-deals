          ...
          <div key={t.key} className="card" style={{
            ...
            <div style={{ padding: "20px 28px 28px", borderTop: "1px solid var(--border)" }}>
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 10 }}>
                {t.features.map(f => (
                  <li key={f} style={{ fontSize: 13, color: "var(--text)", display: "flex", alignItems: "flex-start", gap: 8 }}>
                    <span style={{ color: "#10b981", fontSize: 14 }}>✓</span> {f}
                  </li>
                ))}
                {t.notIncluded.map(f => (
                  <li key={f} style={{ fontSize: 13, color: "var(--text-dim)", display: "flex", alignItems: "flex-start", gap: 8 }}>
            ...
            <div key={i} className="card" style={{ cursor: "pointer", overflow: "hidden" }} onClick={() => setOpenFaq(openFaq === i ? null : i)}>
              ...
              <div style={{
                ...
                <div style={{ overflow: "hidden" }}>
                  <div style={{ padding: "0 24px 18px", fontSize: 13, color: "var(--text-muted)", lineHeight: 1.7 }}>
                    {f.a}
                  </div>
                </div>
              </div>
